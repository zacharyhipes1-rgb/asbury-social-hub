  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-5">
        ...
        {/* Type tabs — flex-wrap so all fit without scrolling */}
        <div className="flex flex-wrap items-center gap-1.5 px-4 sm:px-5 py-3 border-b border-slate-100">
          {TYPE_TABS.map(tab => {
            const Icon = tab.icon
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${typeTab === tab.id ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}
              >
                {Icon && <Icon size={11} />}
                {tab.label}
              </button>
