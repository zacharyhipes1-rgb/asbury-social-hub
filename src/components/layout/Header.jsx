...
import { Bell, LogOut, ChevronDown, Menu, X, Settings, Users, UserCircle, Check, Eye, EyeOff, ShieldAlert } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'
import { useUsers } from '../../context/UsersContext'
import { usePosts } from '../../context/PostsContext'
import { verifyPassword } from '../../utils/auth'
import { notifyProfileChange } from '../../services/emailService'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Current password confirmation — only shown when changing email or password */}
          {needsVerify && (
            <div className="p-3 rounded-xl border border-amber-200 bg-amber-50">
              <div className="flex items-center gap-2 mb-2">
                <ShieldAlert size={13} className="text-amber-600 flex-shrink-0" />
                <p className="text-xs font-semibold text-amber-800">Confirm your current password to save this change</p>
              </div>
              <div className="relative">
                <input type={showCurrent ? 'text' : 'password'} value={currentPw} onChange={e => setCurrentPw(e.target.value)}
                  placeholder="Your current password"
                  className="w-full px-3 py-2.5 pr-10 border border-amber-200 bg-white rounded-xl text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
                <button type="button" onClick={() => setShowCurrent(p => !p)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                  {showCurrent ? <EyeOff size={14} /> : <Eye size={14} />}
                </button>
              </div>
              <p className="text-[11px] text-amber-600 mt-1.5">A security notification will also be sent to your email.</p>
            </div>
          )}
        </div>
...
export default function Header({ onMenuToggle, menuOpen }) {
  ...
  const handleProfileSave = async (form, meta = {}) => {
    const updated = await updateUser(currentUser.id, form)
    if (updated) {
      updateCurrentUser(updated)
      // Send security notification if email or password changed
      if (meta.emailChanged || meta.passwordChanged) {
        try {
          await notifyProfileChange({
            user:            updated,
            changedEmail:    meta.emailChanged,
            changedPassword: meta.passwordChanged,
            oldEmail:        meta.oldEmail,
          })
        } catch {}
      }
    }
