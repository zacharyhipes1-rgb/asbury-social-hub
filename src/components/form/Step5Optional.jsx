import { ChevronLeft, CheckCircle, Users, MessageSquare, Clock, Calendar } from 'lucide-react'
...
export default function Step5Optional({ data, onUpdate, onSubmit, onPrev, isSubmitting }) {
  ...
  const valid = data.scheduled_for
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Provide scheduling and context details, then review your submission before sending for approval.
        </p>
      </div>
      ...
      <div className="space-y-5 mb-8">
        ...
        <div>
          ...
          <input
            ...
            className="w-full sm:w-auto px-4 py-2.5 border border-slate-300 rounded-xl text-sm text-slate-900
              focus:outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all"
          />
          <p className="text-xs text-slate-400 mt-1">Best time to publish for maximum engagement with this audience.</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder:text-slate-400
              focus:outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all"
          />
          <p className="text-xs text-slate-400 mt-1">Helps the team and SEO align targeting. Used for paid boosting later.</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Monthly sales event promotion, model launch, service campaign"
            className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder:text-slate-400
              focus:outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all"
          />
          <p className="text-xs text-slate-400 mt-1">Provides context for your manager during the approval review.</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden mb-6">
        ...
        <div className="px-4 py-1">
          ...
          <ReviewRow label="Target Audience" value={data.target_audience || null} />
          <ReviewRow label="Posting Reason" value={data.posting_reason || null} />
        </div>
      </div>
